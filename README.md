...
| `shortano.link` / `shortino.link` | Partial | ClaimCoin-discovered shortlink hosts are routed to a dedicated handler. Direct HTTP hits Cloudflare; corrected FlareSolverr session reaches `Proxy/VPN Detected`, so an accepted egress/profile lane is still needed. |
...
- `gplinks_http_fast.py` is now the default HTTP-core GPLinks lane (`SHORTLINK_BYPASS_GPLINKS_HTTP_FAST=1` by default). It starts a Turnstile solver prewarm as soon as the engine starts, reuses the token at `/links/go` when the sitekey matches, and still rejects false positives such as `not_enough_steps`.
- `gplinks_live_browser.py` drives the PowerGam 3-step browser flow, scroll/verify handling, pre-navigation network ledger recorder, and final GPLinks Turnstile callback lane. `SHORTLINK_BYPASS_GPLINKS_LIVE_TURNSTILE_PREWARM=1` is default-on and starts the final Turnstile solve while PowerGam is still running, reducing sample `YVTC` to `103.2s` in the latest live probe. `SHORTLINK_BYPASS_GPLINKS_NAVIGATE_FINAL=1` restores old final navigation after a valid final href is exposed. `SHORTLINK_BYPASS_GPLINKS_EARLY_CONTINUE_SECONDS=1` enables the current aggressive PowerGam timer experiment; `2` is known to fail with `not_enough_time` on sample `YVTC`. `SHORTLINK_BYPASS_GPLINKS_HTTP_FINAL_HANDOFF=1` enables the experimental browser-to-HTTP final `/links/go` handoff, currently disabled by default because the live sample returned `Bad Request.` before falling back successfully to browser submit.
