...
OUT = ROOT / "artifacts" / "active" / "gplinks-cdp-diff-latest.json"
...
def run(url: str):
    ...
    try:
        ...
        if urlparse(driver.current_url).netloc.lower() not in POWERGAM_HOSTS and power_url:
        ...
        while time.time() < deadline:
            ...
            if host in GPLINKS_HOSTS and "pid=" in href and "vid=" in href:
                break
            click = click_next_powergam(driver)
            timeline.append({"stage": "power-click", **click})
            drain_perf(driver, events, f"after-click-{len([x for x in timeline if x.get('stage') == 'power-click'])}")
            wait_left = click.get("waitLeft")
            if isinstance(wait_left, (int, float)) and wait_left > 0:
                timeline.append(wait_powergam_continue_ready(driver, min(float(wait_left) + 0.5, 16.0), interval=0.25))
            else:
                wait_document_ready(driver, 4, interval=0.3)
            drain_perf(driver, events, "after-wait")
        candidate = wait_not_cloudflare(driver, 35)
        candidate["stage"] = "candidate"
        timeline.append(candidate)
        drain_perf(driver, events, "candidate")
        rows = summarize(events)
        payload = {"url": url, "decoded": decoded, "candidate_href": candidate.get("href"), "timeline_tail": timeline[-12:], "events": rows}
        OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        print(json.dumps({"out": str(OUT), "candidate_href": candidate.get("href"), "events": len(rows)}, ensure_ascii=False))
    finally:
        try:
            driver.quit()
        except Exception:
