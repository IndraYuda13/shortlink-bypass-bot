...
| `shortano.link` | `partial` | `shortano` | `_handle_shortano_family` | `https://shortano.link/sOxx` | pending | ClaimCoin `/links/go/72` redirected here; direct HTTP/curl_cffi/FlareSolverr currently hit Cloudflare before timer/form mapping. |
| `shortino.link` | `partial` | `shortino` | `_handle_shortano_family` | pending live alias | pending | ClaimCoin wall exposes Shortino, but the exact external alias still needs a live probe; routed through the same partial handler until proven different. |
