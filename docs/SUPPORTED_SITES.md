...
| `shortano.link` | `partial` | `shortano` | `_handle_shortano_family` | `https://shortano.link/sOxx` | pending | ClaimCoin `/links/go/72` redirected here; direct HTTP/curl_cffi hit Cloudflare, and corrected FlareSolverr session mode reaches `Proxy/VPN Detected` before timer/form mapping. |
| `shortino.link` | `partial` | `shortino` | `_handle_shortano_family` | pending live alias | pending | ClaimCoin wall exposes Shortino, but the exact external alias still needs a live probe; routed through the same partial handler and accepted-egress requirement until proven different. |
...
| `gplinks.co` | `live_bypass` | `gplinks` | `_handle_gplinks` | `https://gplinks.co/YVTC` | `http://tesskibidixxx.com` | HTTP-core lane is default-on with Turnstile prewarm before browser fallback; latest proven final URL still came through the PowerGam browser lane and final Turnstile callback. |
